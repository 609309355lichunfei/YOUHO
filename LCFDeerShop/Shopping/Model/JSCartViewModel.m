...
-(NSMutableArray *)selectProduct{
    if (!_selectProduct) {
        _selectProduct = [NSMutableArray arrayWithCapacity:0];
    }
...
-(void)getData{
    ...
    NSMutableArray *    shopSelectAarray = [NSMutableArray arrayWithCapacity:0];
    //创建店铺数据
    for (int i = 0; i < allCount; i++) {
        ...
        NSMutableArray *goodsArray = [NSMutableArray arrayWithCapacity:goodsCount];
         for (int x = 0; x<goodsCount; x++) {
             ...
             cartModel.p_id           = [NSString stringWithFormat:@"%d",arc4random()%10000];
             cartModel.p_price        = [_goodsPicArray[self.random]floatValue];
             cartModel.p_name         = [NSString stringWithFormat:@"%@dkdkdkdk名字很长",@(x)];
        ...
        [shopSelectAarray addObject:@(NO)];
    }
    self.cartData = storeArray;
    self.shopSelectArray = shopSelectAarray;
...
- (void)selectAll:(BOOL)isSelect{
    ...
    if (isSelect) {//全选的话就把所有商品的id全加入，取消全选就全移除
        for (NSArray *array in self.cartData) {
            for (JSCartModel *model in array) {
                [self.selectProduct removeObject:model.p_id];//避免重复添加
                [self.selectProduct addObject:model.p_id];
            }
        }
    }else{
        [self.selectProduct removeAllObjects];
    }
    self.currentSelectCartGoodsCount = _selectProduct.count;
...
- (void)rowSelect:(BOOL)isSelect IndexPath:(NSIndexPath *)indexPath{
    ...
    if (isSelect) {
        ...
        //        for (NSString *ID in self.selectProduct) {
        [self.selectProduct removeObject:model.p_id];
        //        }
...
- (void)deleteGoodsBySingleSlide:(NSIndexPath *)path{
    ...
    [self.selectProduct removeObject:model.p_id];//如果没有这条数据删除也不会报错 所以不管有没有选中 删除就对了
    self.currentSelectCartGoodsCount = _selectProduct.count;
